 ...
 * NOTE: using setDoc overwrites the existing document completely.
 */
export const saveOfficialRoster = async (roster: OfficialStudent[]): Promise<void> => {
    ...
    try {
        ...
        // Overwrite the existing roster with new data and timestamp
        await setDoc(docRef, { 
            students: cleanRoster,
            updatedAt: new Date().toISOString()
        });
        console.log("Roster synced to cloud (Overwritten)");
    } catch (e) {
 ...
 * Returns object with students array and last updated timestamp
 */
export const fetchOfficialRoster = async (): Promise<{ students: OfficialStudent[], updatedAt: string | null }> => {
    if (!db) return { students: [], updatedAt: null };
    try {
        const docRef = doc(db, SETTINGS_COLLECTION, ROSTER_DOC_ID);
        ...
        if (docSnap.exists()) {
            ...
            return {
                students: data.students as OfficialStudent[] || [],
                updatedAt: data.updatedAt || null
            };
        } else {
            return { students: [], updatedAt: null };
        }
    } catch (e) {
